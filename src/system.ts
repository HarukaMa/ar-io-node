...
if (config.ENABLE_SAMPLING_DATA_SOURCE) {
  const innerDataSource = getDataSource(config.SAMPLING_DATA_SOURCE!);
  if (innerDataSource === undefined) {
    throw new Error(
      `Sampling data source '${config.SAMPLING_DATA_SOURCE}' is not configured`,
    );
  }
